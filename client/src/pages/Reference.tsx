...
export default function Reference() {
  ...
  return <main className={`reference-page ref-${active}`}>
    ...
    <section className="compare-section" id="compare"><div className="compare-head"><div><p className="ref-eyebrow">THREE POSSIBLE DIRECTIONS</p><h2>先看气质，<br />再决定结构。</h2></div><p>三种方向都不把你包装成单一职业。影视项目是能力证据，而 IP、内容、写作、AI 和田野共同构成你可以持续发展的合作能力。</p></div><div className="direction-switcher">{(Object.keys(directionData) as Direction[]).map((key) => { const item = directionData[key]; const DirectionIcon = item.icon; return <button className={active === key ? "is-active" : ""} key={key} onClick={() => setActive(key)}><DirectionIcon size={17} /><span>{item.label}</span><small>{item.promise}</small></button>; })}</div></section>

    <section className={`direction-preview preview-${active}`}><div className="preview-rail"><span>0{active === "partner" ? 1 : active === "lab" ? 2 : 3}</span><span>{data.label}</span><span className="preview-rail-line" /><span>BINGJING<br />BIAN</span></div><div className="preview-main"><div className="preview-hero"><div className="preview-hero-copy"><p className="preview-label"><Icon size={14} /> {data.label}</p><h2>{data.title}</h2><p>{data.subtitle}</p><div className="preview-actions"><span>查看项目 <ArrowUpRight size={14} /></span><span>谈一个合作 <ArrowUpRight size={14} /></span></div></div><div className="preview-hero-art"><div className="art-frame"><img src={assetBase + "editorial-hero.webp"} alt="界面参考主视觉" /><span>FIELD NOTE / 001<br /><small>QUANZHOU · 2024.06</small></span></div><div className="art-circle" /><div className="art-word">B·B·J</div></div></div><div className="preview-proof"><span>合作方可以找我做什么</span><em className="proof-field-note">从现场材料里找到真正的问题，再决定内容如何发生。</em><div><strong>01</strong><b>老板 IP 起号与表达培训</b><small>把个人经验变成可持续的内容系统</small></div><div><strong>02</strong><b>品牌内容与新媒体项目</b><small>从策略、脚本到现场与发布协作</small></div><div><strong>03</strong><b>影视协作、短剧与 AI 实验</b><small>把故事、人物和新工具放进真实项目</small></div></div><div className="preview-projects"><div className="preview-section-title"><span>SELECTED PROJECTS</span><a href="#contact">全部项目 <ArrowUpRight size={13} /></a></div><div className="preview-project-grid"><article className="preview-project project-feature"><div className="project-visual visual-red"><span>01</span><b>《台魁巷》</b><small>纪录片 / 跟组制片与录音</small><i>泉州 · 人物关系 · 现场材料</i></div><p>长期进入现场，把地方经验转成可以被观看的内容。<br /><small>我的角色：跟组制片 / 录音协作</small></p></article><article className="preview-project"><div className="project-visual visual-photo"><img src={assetBase + "project-archive.webp"} alt="项目过程参考" /><span>02</span><i>项目材料 / 观察 006</i></div><p>老板 IP 起号与培训<br /><small>内容策略 / 表达训练 / 陪跑</small></p></article><article className="preview-project"><div className="project-visual visual-green"><span>03</span><b>枫林游魂</b><small>实景互动游戏 / 村庄联动</small><i>地方 · 故事 · 参与关系</i></div><p>把地方、故事与参与感放进一次真实体验。<br /><small>项目类型：实景互动游戏 / 村庄联动</small></p></article></div></div><div className="preview-lower"><div><span className="preview-kicker">THE LONG VIEW / FIELD TRACE</span><h3>从旅游与自然教育，<br />走到内容、IP与 AI 实验。</h3><small className="field-trace">泉州 · 在地观察 · 现场经验</small></div><div className="preview-timeline"><span>2017—2022</span><b>纪录片与影视现场</b><span>2023—现在</span><b>IP 主理与跨领域内容</b><span>接下来</span><b>短剧、AI 与新的合作</b></div></div><div className="preview-contact" id="contact"><div><span className="preview-kicker">OPEN INVITATION</span><h3>如果你有一个<br /><em>需要被说清楚的问题，</em><br />可以来聊聊。</h3></div><a href="mailto:875569322@qq.com"><Mail size={17} /> 875569322@qq.com <ArrowUpRight size={16} /></a></div></div></section>
